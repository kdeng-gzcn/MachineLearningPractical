...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
                 test_data, weight_decay_coefficient, use_gpu, continue_from_epoch=-1, lr=1e-3): # add arg lr
        """
        ...
        print('Learning Rate', lr) # print lr
        ...
        self.optimizer = optim.Adam(self.parameters(), amsgrad=False,
                                    weight_decay=weight_decay_coefficient, lr=lr) # lr = 1e-3 / 1e-2
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        for name, param in named_parameters:
            ...
            if "weight" in name: # VGP usually happens in weight, hence we record the gradient of the weight in each network layer

                weight_name = "_".join(part for part in name.split('.') if part not in ("layer_dict", "weight")) # simplify layer name for visualization later
                abs_mean_grad = param.grad.abs().mean().item() # compute absolute mean of gradient

                all_grads.append(abs_mean_grad)
