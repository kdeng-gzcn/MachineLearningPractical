...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            if self.share_across_batch:
                # if share
                mask_in_batch = self.rng.uniform(0, 1, size=(1, *inputs.shape[1:])) < self.incl_prob
                self.mask = np.repeat(mask_in_batch, inputs.shape[0], axis=0)
            else:
                # else use unique mask for each batch
                self.mask = self.rng.uniform(0, 1, size=inputs.shape) < self.incl_prob
        else:
            # In inference, we scale the data
            self.mask = np.full(inputs.shape, self.incl_prob)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, input_dim).
        """
        grads_wrt_inputs = grads_wrt_outputs * self.mask
